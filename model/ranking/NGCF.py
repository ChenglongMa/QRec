...
class NGCF(DeepRecommender):
    ...
    def initModel(self):
        ...
        for k in range(self.n_layers):
            side_embeddings = tf.sparse.sparse_dense_matmul(norm_adj, ego_embeddings)
            ...
            def dropout():
                return tf.nn.dropout(ego_embeddings, rate=1 - (0.9))

            ego_embeddings = tf.cond(pred=self.isTraining, true_fn=lambda: dropout(),
                                     false_fn=lambda: without_dropout())
        ...
        self.test = tf.reduce_sum(input_tensor=tf.multiply(self.u_embedding, self.multi_item_embeddings), axis=1)
