...
if __name__ == '__main__':
    ...
    for n in range(3):
        ...
        for i in range(11):
            for on in 'pre during'.split():
                for n_normal_users in [25]:
                    output_dir = f'./results/output_{n}/n_normal_users_{n_normal_users}'
                    if enable_rating_task:
                        ...
                        for algo in 'SlopeOne SocialMF RSTE SVD ItemKNN UserKNN ItemMean SVD++'.split():  # TODO: change algorithm list
                            ...
                            is_rank = False
                            cmds.append(f'python run.py {n} {i} {algo} {on} {is_rank} -1 {n_normal_users}')
                    ...
                    if enable_ranking_task:
                        ...
                        for algo in 'Rand MostPopular ItemMean SVD SVD++ BPR LightGCN CFGAN NeuMF'.split():
                            ...
                            is_rank = True
                            top_n = 10
                            cmds.append(f'python run.py {n} {i} {algo} {on} {is_rank} {top_n} {n_normal_users}')
    ...
    if async_run:
        jobs = np.array(cmds).reshape((-1, min(22, len(cmds))))
        for job in jobs:
            print('To run:')
            print(job)
