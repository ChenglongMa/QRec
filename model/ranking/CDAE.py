...
class CDAE(DeepRecommender):
    ...
    def initModel(self):
        ...
        self.X = tf.compat.v1.placeholder(tf.float32, [None, self.num_items])
        self.positive = tf.compat.v1.placeholder(tf.float32, [None, self.num_items])
        self.negative = tf.compat.v1.placeholder(tf.float32, [None, self.num_items])
        self.V = tf.Variable(initializer([self.num_users, self.n_hidden]))
        self.U_embeding = tf.nn.embedding_lookup(params=self.V, ids=self.u_idx)
        self.weights = {
    ...
    def buildModel(self):
        ...
        self.loss = -tf.multiply(y_positive, tf.math.log(y_pred)) - tf.multiply((y_negative), tf.math.log(1 - y_pred))
        ...
        self.loss = tf.reduce_mean(input_tensor=self.loss) + self.reg_loss
        ...
        for epoch in range(self.maxEpoch):
            ...
            _, loss = self.sess.run([optimizer, self.loss], feed_dict={self.X: batch_xs, self.mask_corruption: mask,
                                                                       self.u_idx: users, self.positive: positive,
