...
class EE(IterativeRecommender):
    ...
    def buildModel_tf(self):
        ...
        global_mean = tf.compat.v1.placeholder(tf.float32, [None], name="mean")
        reg_lambda = tf.constant(self.regU, dtype=tf.float32)
        reg_biase = tf.constant(self.regB, dtype=tf.float32)
        self.U_bias = tf.Variable(tf.random.truncated_normal(shape=[self.num_users], stddev=0.005, mean=0.02),
                                  name='U_bias')
        self.V_bias = tf.Variable(tf.random.truncated_normal(shape=[self.num_items], stddev=0.005, mean=0.02),
                                  name='V_bias')
        self.U_bias_embed = tf.nn.embedding_lookup(params=self.U_bias, ids=self.u_idx)
        self.V_bias_embed = tf.nn.embedding_lookup(params=self.V_bias, ids=self.v_idx)
        difference = tf.subtract(self.user_embedding, self.item_embedding)
        self.r_hat = tf.reduce_sum(input_tensor=tf.multiply(difference, difference), axis=1)
        ...
        optimizer = tf.compat.v1.train.AdamOptimizer(self.lRate)
