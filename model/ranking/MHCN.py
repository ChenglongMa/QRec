    ...
    def initModel(self):
        ...
        def channel_attention(*channel_embeddings):
            ...
            for embedding in channel_embeddings:
                weights.append(tf.reduce_sum(input_tensor=tf.multiply(self.weights['attention'], tf.matmul(embedding,
                                                                                                           self.weights[
                                                                                                               'attention_mat'])),
                                             axis=1))
            score = tf.nn.softmax(tf.transpose(a=weights))
            mixed_embeddings = 0
            for i in range(len(weights)):
                mixed_embeddings += tf.transpose(
                    a=tf.multiply(tf.transpose(a=score)[i], tf.transpose(a=channel_embeddings[i])))
        ...
        user_embeddings_c1 = tf.reduce_sum(input_tensor=all_embeddings_c1, axis=0)
        user_embeddings_c2 = tf.reduce_sum(input_tensor=all_embeddings_c2, axis=0)
        user_embeddings_c3 = tf.reduce_sum(input_tensor=all_embeddings_c3, axis=0)
        simple_user_embeddings = tf.reduce_sum(input_tensor=all_embeddings_simple, axis=0)
        item_embeddings = tf.reduce_sum(input_tensor=all_embeddings_i, axis=0)
        ...
        # embedding look-up
        self.batch_neg_item_emb = tf.nn.embedding_lookup(params=self.final_item_embeddings, ids=self.neg_idx)
        self.batch_pos_user_emb = tf.nn.embedding_lookup(params=self.final_user_embeddings, ids=self.u_idx)
        self.batch_pos_item_emb = tf.nn.embedding_lookup(params=self.final_item_embeddings, ids=self.v_idx)

    def hierarchical_self_supervision(self,em,adj):
        def row_shuffle(embedding):
            return tf.gather(embedding, tf.random.shuffle(tf.range(tf.shape(input=embedding)[0])))

        def row_column_shuffle(embedding):
            corrupted_embedding = tf.transpose(a=tf.gather(tf.transpose(a=embedding), tf.random.shuffle(
                tf.range(tf.shape(input=tf.transpose(a=embedding))[0]))))
            corrupted_embedding = tf.gather(corrupted_embedding,
                                            tf.random.shuffle(tf.range(tf.shape(input=corrupted_embedding)[0])))
            return corrupted_embedding

        def score(x1, x2):
            return tf.reduce_sum(input_tensor=tf.multiply(x1, x2), axis=1)
        user_embeddings = em
        # user_embeddings = tf.math.l2_normalize(em,1) #For Douban, normalization is needed.
        edge_embeddings = tf.sparse.sparse_dense_matmul(adj, user_embeddings)
        #Local MIM
        pos = score(user_embeddings,edge_embeddings)
        neg1 = score(row_shuffle(user_embeddings),edge_embeddings)
        neg2 = score(row_column_shuffle(edge_embeddings), user_embeddings)
        local_loss = tf.reduce_sum(
            input_tensor=-tf.math.log(tf.sigmoid(pos - neg1)) - tf.math.log(tf.sigmoid(neg1 - neg2)))
        #Global MIM
        graph = tf.reduce_mean(input_tensor=edge_embeddings, axis=0)
        pos = score(edge_embeddings, graph)
        neg1 = score(row_column_shuffle(edge_embeddings), graph)
        global_loss = tf.reduce_sum(input_tensor=-tf.math.log(tf.sigmoid(pos - neg1)))
        return global_loss+local_loss

    def buildModel(self):
        y = tf.reduce_sum(input_tensor=tf.multiply(self.batch_pos_user_emb, self.batch_pos_item_emb), axis=1) \
            - tf.reduce_sum(input_tensor=tf.multiply(self.batch_pos_user_emb, self.batch_neg_item_emb), axis=1)
        reg_loss = 0
        for key in self.weights:
            reg_loss += 0.001 * tf.nn.l2_loss(self.weights[key])
        rec_loss = -tf.reduce_sum(input_tensor=tf.math.log(tf.sigmoid(y))) + \
